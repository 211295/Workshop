...
| _[pwd](https://man7.org/linux/man-pages/man1/pwd.1.html)_ | **P**rint **W**ork **D**irectory :computer: = Mostra qual o _Diretório_/_Pasta_ atual de trabalho |  | Garante o nome correto do _Diretório_/_Pasta_ e do o caminho atual de trabalho |
| _[cp](https://man7.org/linux/man-pages/man1/mkdir.1.html)_ | **C**o**P**y :copyright: = copia arquivos ou _Diretório_/_Pasta_ | `-t` | Importante lembrar que a cópia deve haver um nome diferente caso esteja presente no mesmo _Diretório_/_Pasta_ |
...
| _[touch](https://man7.org/linux/man-pages/man1/touch.1.html)_ | Cria um arquivo em branco :book: |  | Pode-se criar um arquivo novo com intuito de garantir que o _output_ sejam indexados ao arquivo novo (utilizando o sinal `>`). |
| _[nano](https://linux.die.net/man/1/nano)_ | **N**ano's **ANO**ther editor :black_nib: = Edita um arquivo texto (e até mesmo cria caso não exista) |  | Após a edição, deve-se apertar `CRTL` + `X`, e depois `Y` para salvar as alterações. |
...
| _[colrm](https://man7.org/linux/man-pages/man1/colrm.1.html)_ | **COL**umn **R**e**M**ove = Remove a coluna de strings selecioanda |  | Utiliza-se dos outputs de outros arquivos e necessita-se dos números  |
